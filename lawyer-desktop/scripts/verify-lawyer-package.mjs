...
export default async function verify(context) {
  ...
  for(const file of ['platform.tgz','brand.tgz','market.tgz','genui.tgz','materials.tgz','builtin-plugins.json']) {
  ...
  for (const [sourceName, targetName] of [['LICENSE', 'Electron-LICENSE.txt'], ['LICENSES.chromium.html', 'LICENSES.chromium.html']]) {
    const expected=readFileSync(join(context.packager.projectDir,'node_modules/electron/dist',sourceName))
    const actual=readFileSync(join(resources,'licenses',targetName))
    if(!expected.equals(actual))throw new Error('Electron/Chromium license mismatch '+targetName)
  }

...
const seen = new Set()
const manifests = []
const queue = [{ name: rootManifest.name ?? 'lawyer-dsh-desktop', manifestPath: join(packageRoot, 'package.json') }]
...
if (noticesArg !== -1) {
  ...
  const lines = [
    '# Third-Party Notices',
    '',
    'LawyerDesk distributes the following third-party packages inside its installers.',

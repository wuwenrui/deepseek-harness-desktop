...
describe('packaged dsh bootstrap', () => {
  ...
  it('clears Node mode and dispatches the imported CLI exactly once', async () => {
    ...
    const argv = ['/Applications/LawyerDesk', '/app.asar/lib/desktop-cli.js', '--dump-config']
    const runCli = vi.fn(async () => {})
    const load = vi.fn(async (url: string) => {
      expect(environment).toEqual({ KEEP: 'value' })
      expect(argv).toEqual([
        '/Applications/LawyerDesk',
  ...
  it('uses the physical unpacked dependency tree only inside an Electron package', () => {
    expect(unpackedAsarPath('/Applications/LawyerDesk.app/Contents/Resources/app.asar/node_modules/pkg'))
      .toBe('/Applications/LawyerDesk.app/Contents/Resources/app.asar.unpacked/node_modules/pkg')
    expect(unpackedAsarPath('C:\\Program Files\\LawyerDesk\\resources\\app.asar\\node_modules\\pkg'))
      .toBe('C:\\Program Files\\LawyerDesk\\resources\\app.asar.unpacked\\node_modules\\pkg')
    expect(unpackedAsarPath('/Applications/LawyerDesk.app/Contents/Resources/app.asar/package.json'))
      .toBe('/Applications/LawyerDesk.app/Contents/Resources/app.asar.unpacked/package.json')
  ...
  it('resolves the DSH entry from a pnpm profile with flat package dependencies', () => {
    ...
    const desktopLib = join(root, 'node_modules', 'lawyer-dsh-desktop', 'lib')

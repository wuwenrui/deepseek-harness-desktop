...
describe('published package surface', () => {
  ...
  it('runs desktop and community market typechecks from the root command', () => {
    expect(workspaceManifest.scripts?.typecheck)
      .toBe('yarn workspace dsh-plugin-desktop typecheck && yarn workspace lawyer-dsh-desktop typecheck && yarn workspace dsh-community-market typecheck')
  })

  it('runs desktop and community market tests from the root command', () => {
    expect(workspaceManifest.scripts?.test)
      .toBe('yarn workspace dsh-plugin-desktop test && yarn workspace lawyer-dsh-desktop test && yarn workspace dsh-community-market test')
  })

  it('registers both npm launcher names', () => {
    expect(manifest.name).toBe('lawyer-dsh-desktop')
    expect(manifest.bin).toEqual({
      'lawyer-dsh-desktop': 'lib/bin.js',
      'lawyer-desktop': 'lib/bin.js',
    })
  ...
  it('exposes the Host plugin and desktop-owned client face', () => {
    ...
    expect(readFileSync(new URL('cordis.patch.yml', packageRoot), 'utf8')).toContain('name: lawyer-dsh-desktop')
    expect(readFileSync(new URL('cordis.patch.yml', packageRoot), 'utf8')).not.toContain('name: dsh-community-market')
    expect(readFileSync(new URL('cordis.patch.yml', packageRoot), 'utf8')).toContain('name: lawyer-dsh-desktop/terminal')
    expect(readFileSync(new URL('cordis.patch.yml', packageRoot), 'utf8')).toContain('name: lawyer-dsh-desktop/pnpm')
    expect(readFileSync(new URL('cordis.patch.yml', packageRoot), 'utf8')).toContain('name: lawyer-dsh-desktop/profiles')
    expect(readFileSync(new URL('cordis.patch.yml', packageRoot), 'utf8')).toContain('name: lawyer-dsh-desktop/diagnostics')
    expect(readFileSync(new URL('cordis.patch.yml', packageRoot), 'utf8')).toContain('name: lawyer-dsh-desktop/notifications')
    expect(readFileSync(new URL('cordis.patch.yml', packageRoot), 'utf8')).toContain('name: lawyer-dsh-desktop/updates')
  })
  ...
  it('separates unsigned smoke packaging from the signed macOS release', () => {
    ...
    expect(workspaceManifest.scripts?.['dist:mac:beta'])
      .toBe('yarn aa:prepare-release && yarn workspace dsh-community-market build && yarn workspace lawyer-dsh-desktop dist:mac')
    expect(workspaceManifest.scripts?.['dist:mac-smoke:beta'])
      .toBe('yarn aa:prepare-release && yarn workspace dsh-community-market build && yarn workspace lawyer-dsh-desktop dist:mac-smoke')
    expect(workspaceManifest.scripts?.['dist:win:beta'])
      .toBe('yarn aa:prepare-release && yarn workspace dsh-community-market build && yarn workspace lawyer-dsh-desktop dist:win')
    expect(workspaceManifest.scripts?.['dist:win-portable:beta'])
      .toBe('yarn aa:prepare-release && yarn workspace dsh-community-market build && yarn workspace lawyer-dsh-desktop dist:win-portable')
  ...
  it('runs platform package gates before reusing native packaging outputs', () => {
    ...
    expect(windowsJob).toContain('workspace: [dsh-plugin-desktop, lawyer-dsh-desktop]')
    ...
    expect(macosJob).toContain('workspace: [dsh-plugin-desktop, lawyer-dsh-desktop]')

...
describe('desktop update installer download', () => {
  it('pins a Beta artifact request to its channel and target version', async () => {
    ...
    const destination = join(directory, 'LawyerCopilot-2.0.5-beta.2-mac.dmg')
    ...
    expect(desktopUpdateFilename('darwin', '2.0.5-beta.2', 'beta'))
      .toBe('LawyerCopilot-2.0.5-beta.2-mac.dmg')
  })
  ...
  it('accepts a Beta artifact without response identity headers', async () => {
    ...
    const result = await downloadDesktopUpdate({
      ...
      version: '2.0.5-beta.2',
      channel: 'beta',
      destinationPath: join(directory, 'LawyerCopilot-2.0.5-beta.2-mac.dmg'),
      request: async () => chunkedResponse([dmgArtifact()]),
    })
    expect(result).toBe(join(directory, 'LawyerCopilot-2.0.5-beta.2-mac.dmg'))
  })

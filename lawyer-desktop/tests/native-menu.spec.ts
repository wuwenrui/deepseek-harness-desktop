...
describe('native macOS application menu', () => {
  ...
  it('keeps the English fallback complete', () => {
    const template = macApplicationMenuTemplate('LawyerDesk', 'en')

    expect(template.map(item => item.label)).toEqual([
      'LawyerDesk', 'File', 'Edit', 'View', 'Window',
    ])
    expect(submenu(template[0]!)).toEqual(expect.arrayContaining([
      expect.objectContaining({ label: 'About LawyerDesk', role: 'about' }),
      expect.objectContaining({ label: 'Quit LawyerDesk', role: 'quit' }),
    ]))
  })

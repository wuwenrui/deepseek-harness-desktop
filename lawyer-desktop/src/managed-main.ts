...
import { finalizeProductSeeds, rollbackProductSeeds } from '@lawyer-dsh/lawyer-platform/product-seeds'
...
function stop(code: number): Promise<void> {
  ...
  shutdown = (async () => {
    try { await ctx?.fiber.dispose() } finally {
      if (code !== 0) {
        try { await rollbackProductSeeds(join(home, 'profiles', PRODUCT_PROFILE)) }
        catch { console.error('lawyer-desktop: 升级恢复未完成，已保留原环境与恢复记录') }
      }
      releaseInstance?.(); app.exit(code)
    }
  })()
  return shutdown
}
...
async function main(): Promise<void> {
  ...
  callbacks.clear()
  const rendered = runtime.beginRendererBootMonitoring({ commitHealthy: async () => { await finalizeProductSeeds(directory) } })

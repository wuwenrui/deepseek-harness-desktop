/** Isolated command-line environment launched from the LawyerDesk tray. */
...
/** Platforms with a native terminal launch contract owned by LawyerDesk. */
...
function macWelcome(
  ...
  return [
    ...
    `printf '%s\\n' ${quoteSh(`LawyerDesk ${options.productVersion} terminal`)}`,
    ...
    `printf '%s\\n' ${quoteSh('Restart LawyerDesk after plugin changes.')}`,
...
function windowsWelcome(): string {
  ...
  return [
    ...
    `Write-Host ("LawyerDesk {0} terminal" -f $env:${WINDOWS_PRODUCT_VERSION})`,
    ...
    `Write-Host 'Restart LawyerDesk after plugin changes.'`,
    '',
  ].join('\r\n')
}
...
function windowsCmdWelcome(): string {
  ...
  return [
    ...
    `echo(LawyerDesk !${WINDOWS_PRODUCT_VERSION}! terminal`,
    ...
    `echo(${escapeBatchText('Restart LawyerDesk after plugin changes.')}`,

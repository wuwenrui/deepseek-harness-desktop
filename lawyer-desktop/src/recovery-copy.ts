...
const COPY: Record<DesktopLocale, DesktopRecoveryCopy> = {
  ...
  zh: {
    title: '律衡恢复助手',
    fallbackBody: '无法读取恢复信息。请退出并重新启动律衡。',
    reason: '进入恢复模式的原因',
    ...
    confirmRestoreDefaultDirectoryBody: '律衡将改为使用当前系统的默认数据目录。当前数据目录不会被删除。',
    ...
    confirmCreateDefaultDirectoryBody: '律衡将在默认路径创建一个全新的环境并重启。当前数据目录不会被删除。',
    confirmCreateDefaultDirectoryAction: '新建并重启',
    confirmFactoryReset: '恢复律衡出厂设置？',
    confirmFactoryResetMessage: '移除当前 DSH 数据并重新初始化？',
    confirmFactoryResetBody: currentDirectory => `以下目录将被移入系统废纸篓或回收站：\n\n${currentDirectory}\n\n其中保存的 Profile、插件、设置、凭据、会话、附件和工作区记录都会从律衡中移除；不会删除此目录以外的项目文件。随后律衡会重启并创建干净的默认 Profile。`,
    ...
    confirmRollbackBody: capturedAt => `将立即恢复 ${capturedAt} 创建的检查点中的 Profile、共享设置文件 settings.yaml 与 DSH 数据目录补丁；重启后，律衡将使用回滚后的配置。`,
    confirmRollbackAction: '恢复配置',
    uninstalledSuccess: '插件已从当前 Profile 中卸载。请重启律衡以使用更新后的插件配置。',
    rollbackSuccess: slotId => `已恢复检查点 ${slotId}。重启后将使用恢复的配置；首次成功启动时会保留现有检查点。`,
    profileSelectedSuccess: '已设为当前 Profile。请重启律衡以使用该 Profile。',

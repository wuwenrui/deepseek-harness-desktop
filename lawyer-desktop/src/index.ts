/** LawyerDesk Host plugin: owns the selected native shell generation. */
...
export function apply(ctx: Context, config: Config): void {
  ...
  if (runtime === undefined) {
    process.stderr.write(
      'dsh-plugin-desktop: this profile is composed with the LawyerDesk shell, which requires the desktop launcher (desktopRuntime).\n'
      + 'Start it with `dsh-desktop`, or select this profile inside the packaged LawyerDesk application.\n'
  ...
  const settings = ctx.settings.register(
    ...
    DesktopSettingsSchema,
    {
      applies: 'restart',
      validate: (value) => {
        if (value.mode !== 'compatibility' || value.networkExposure !== 'loopback' || value.openBrowser) throw new Error('受管桌面不允许切换布局、开放局域网或公开浏览器访问')
  ...
  if (runtime.platform !== 'linux') { // 系统选择框（含 macOS）自带分栏与搜索，两个桌面平台都注册原生选择路由
    ctx.effect(
      () => ctx.webServer.register({
        kind: 'exact',

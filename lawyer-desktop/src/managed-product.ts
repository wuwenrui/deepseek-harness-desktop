...
import { PRODUCT_BUNDLES, composeProductBundles } from '@lawyer-dsh/lawyer-platform/builtin-plugins'
import { installProductSeeds } from '@lawyer-dsh/lawyer-platform/product-seeds'
...
export function syncProductBundles(profile: string): void {
  ...
  const composed = composeProductBundles(bundles)
...
export async function prepareManagedProduct(home: string): Promise<string> {
  ...
  await installProductSeeds({
    profile, seedDirectory: seed, deferCommit: true,
    install: async (stage) => {
      validateManagedProfile(stage)
      await runOwned({ ...command, args: [...command.args, 'install', '--no-frozen-lockfile', '--ignore-scripts', '--config.auto-install-peers=false'] }, stage, safeEnvironment())
    },
  })

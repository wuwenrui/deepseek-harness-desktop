...
import type { DesktopWindowService } from 'lawyer-dsh-desktop/client'
...
import type {
  ...
  DesktopProfiles,
} from 'lawyer-dsh-desktop/profile-service'
import type {
  DesktopPnpm,
  DesktopPnpmHandle,
  DesktopPnpmOutcome,
} from 'lawyer-dsh-desktop/pnpm'
```

`lawyer-dsh-desktop/profiles` is the Desktop-owned tray consumer, not the profile service contract. Do not import it for service types.
...
| --- | --- | --- |
| `desktopProfiles` | Generation-scoped Host service. | Public and supported through `lawyer-dsh-desktop/profile-service`. |
| `desktopPnpm` | Generation-scoped Host service. | Public and supported through `lawyer-dsh-desktop/pnpm`. |
| `desktopWindow` | Generation-scoped Client service. | Public and supported through `lawyer-dsh-desktop/client`; immutable geometry only. |
...
import type {} from 'lawyer-dsh-desktop/profile-service'
import type { DesktopPnpmHandle } from 'lawyer-dsh-desktop/pnpm'
...
import type { Context } from '@deepseek-ai/cordis'
import type {} from 'lawyer-dsh-desktop/profile-service'
import type {} from 'lawyer-dsh-desktop/pnpm'
...
Type-only imports are erased from JavaScript. A cross-environment package can keep `lawyer-dsh-desktop` as a development dependency for compilation, or as an optional peer if it publishes declarations that expose these types; it does not need a runtime import merely to probe the services.
...
```sh
yarn workspace lawyer-dsh-desktop build
yarn workspace lawyer-dsh-desktop verify:profile
```

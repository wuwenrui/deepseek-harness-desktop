...
- `@lawyer-dsh/lawyer-platform/builtin-plugins` owns the fixed `PRODUCT_BUNDLES`; `src/managed-product.ts` consumes it rather than duplicating it. Agent Teams remains enabled: experimental host and Web layers apply after `dsh-base`/`dsh-web-app`. The reviewed legal GenUI/materials seeds are mandatory too. Shared product-seed installation preserves market-appended bundles, defers commit until renderer/native health and recovers interrupted upgrades. Removing a mandatory layer is a policy violation, not an upgrade path.
